			...
			<div class="contentList clearfix">
				...
				<div class="zffs">
					<ul>
						...
						<li class="useCard">
							<p v-tap='{methods:selectCards,index:1}'>优惠劵 <span class=""><i class="icon iconfont">&#xe639;</i></span></p>
						...
						<li class="userGifts" v-show="giftsCard">
							<p>使用礼品卡金额
								<span v-if="giftcardBalance > 0">-￥{{total >= giftcardBalance ? giftcardBalance:total | toFixedTwo}}</span>
								<span v-else>-￥0</span>
							</p>
...
<script>
   ...
   import Api from '../../API.js'
    export default {
			...
			Api.car.createOrder(jsons).then(res=>{
				if(res.data.code == 'success'){
                    ...
                    var isZeroPaid = res.data.isZeroPaid
          			this.$router.push( { path : '/orderStatus', query : { 'paymentType' : 'wx', 'addressId' : this.addressData.dbId, 'dbId' : orderDbId, 'userDbId' : userDbId, 'openId' : openId, 'client' : 'mobile', 'sorce' : 'payOrder','isZeroPaid':isZeroPaid }});
				}else{
		...
		useCoupon(){
			this.$router.push({path:"/UseCoupon",query:{'openId':this.$route.query.openId,'orderDbId':this.$route.query.orderDbId,'userDbId':localStorage.getItem("userDbId"),couponPrice:this.couponCode}})
