		...
		<div class="order" v-for="(order, indexs) in dataList">
			<ul>
				<li>
					...
					<span v-if='order.status == -1' v-tap="{methods:delectFn,index:indexs,dbid:order.dbId,code:order.code}"><i class="icon iconfont">&#xe68a;</i></span>
				</li>
				<li>
					<span>状态</span>
					<span>{{order.orderState}}</span>
				</li>
				<li class="ordersContent" v-for="(works, index) in order.worksList">
				...
				<li>
					<div>
						<p v-bind:hidden="order.status != 1" v-tap="{methods:cancleOrder,dbId:order.dbId,index:indexs,code:order.code}"> 取消订单</p>
        ...
        methods: {
            ...
            Api.car.cloneOrder(ordJson).then(res=>{
                 if(res.data.code == 'success'){
					...
					this.$router.push( { path : "/orderStatus",query:{'paymentType':'wx','addressId':addressDbId,"dbId":orderDbId,"userDbId":userDbId,"openId":openId,'sorce':'payOrder'} } )
        	...
        	Api.car.orderListStatus({userDbId:localStorage.getItem("userDbId")}).then(res=>{
        		...
        		for(var i=0; i<this.dataList.length; i++){
        			if(this.dataList[i].cardDiscountAmount > 0){
	        			...
	        			//alert(this.dataList[i].couponDiscountAmount)
