	...
	<div id="bbs-select">
		...
		<div class="bbsImg">
			...
			<dl v-for="(item,index1) in bbs.attributes"  class="slect_dl">
				...
				<dd>
					<div v-for="(elements,index2)  in item.attributeValues"  :data-name="elements.name" :data-code="elements.code" v-tap="{methods : check,index1:index1,index2:index2}"  :class="elements.colorF? 'dd_slect dd_slectWidth dd_active' : 'dd_slect dd_slectWidth '">
						<b  v-bind:style="{background:elements.attr}" v-if="item.code =='color'"></b>
						{{elements.name}}
					</div>
				</dd>
			</dl>
...
//				 var s1 = res.data.attributes[0]
//				 var s2 = res.data.attributes[1]
//				 var s3 = res.data.attributes[2]
//				 var arr = [];
//				 arr.push(s2) //页数
//				 arr.push(s3) //尺寸
//				 arr.push(s1) //颜色
